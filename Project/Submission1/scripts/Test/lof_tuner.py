...
class LOF_AutoTuner(object):
    def __init__(self, n_samples=500, data=None, c_max=0.1, k_max=100):
        ...
        self.c_grid = np.linspace(
            0.005, self.c_max, self.c_steps)  # contamination
    ...
    def visualise(self):
        ...
        for i, v in tqdm.tqdm(enumerate(self.collector)):
            ...
            ax[0, 0].text(0.05, 0.85, 'Z :' + str(Z), c='b',
                          transform=ax[0, 0].transAxes)

            ax[1, 0].scatter(c_lis, k_lis, c='r', s=5.)
            ax[1, 0].text(0.05, 0.85, 'K :' + str(Kopt),
                          c='r', transform=ax[1, 0].transAxes)
            ...
            if Kopt == self.tuned_params['k'] and contamination == self.tuned_params['c']:
                ax[1, 1].scatter(self.data[:, 0], self.data[:, 1],
                                 facecolors='none', s=1000 * log_lof, edgecolors='darkgray')

            ax[1, 1].scatter(np.ma.masked_where(log_lof < thresh, self.data[:, 0]), np.ma.masked_where(
                log_lof < thresh, self.data[:, 1]), c='orange', s=5.0)
            ax[1, 1].scatter(np.ma.masked_where(log_lof > thresh, self.data[:, 0]), np.ma.masked_where(
                log_lof > thresh, self.data[:, 1]), c='green', s=5.0)
            ax[1, 1].text(0.05, 0.85, 'C :' + str(contamination),
                          c='darkgray', transform=ax[1, 1].transAxes)
            cam.snap()
        ...
        for contamination in tqdm.tqdm(self.c_grid):
            ...
            for k in self.k_grid:
                clf = LocalOutlierFactor(
                    n_neighbors=k, contamination=contamination)
                clf.fit_predict(self.data)
                ...
                vc_in = np.var(x_in)
                Tck = (mc_out - mc_in) / \
                    np.sqrt((self.eps + ((1/samps)*(vc_out + vc_in))))
